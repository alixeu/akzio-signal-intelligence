...
3. **If it already exists**, check whether the user has explicitly confirmed the same target, exclusion rules, and scanning method in the available task context.
   - If confirmed and unchanged, state that the confirmed scope is being reused and continue without asking again.
   - If not previously confirmed, show the effective scope and wait for confirmation. File existence alone is not authorization.
   - If the confirmed scope has materially changed, show the difference and confirm the changed part before scanning it.
4. After obtaining or reusing applicable explicit confirmation, proceed to Phase 1. Do not infer approval from silence.
