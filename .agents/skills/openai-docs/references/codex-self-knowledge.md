...
For narrow Codex documentation questions, search and fetch the relevant official page before relying on its product claims. Do not turn a targeted documentation lookup into broad manual synthesis. For local configuration, instruction-file reviews, and error diagnosis, inspect local evidence first; consult official documentation when a conclusion depends on current documented behavior. Neither documentation nor manual retrieval blocks independent local work. Current or latest model questions follow the model-selection route.
