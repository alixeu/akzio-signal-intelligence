...
- Never modify `scripts/image_gen.py`. If a required capability is missing, inspect the capabilities of the already-authorized path and continue unaffected work. Ask only when the missing capability blocks a required deliverable or an alternative needs additional authorization. Preserve the explicit CLI/API fallback and model-switch approvals above.
