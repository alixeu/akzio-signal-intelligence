...
description: Audit a codebase's animation and motion, then produce prioritized findings and implementation plans. Use for requested motion audits, roadmaps, or plan execution. A general request to improve or fix an interface is an implementation task; this skill's audit stage must not replace that deliverable.
---
...
The audit stage surveys animation and motion code and produces prioritized findings and implementation plans. It does not review a single diff (that's `review-animations`). Keep an audit-only request read-only. When the user has authorized implementation, finish the necessary analysis and continue into implementation and verification in the same task; no special invocation or repeated request is needed.
...
1. **Keep the audit stage read-only on source.** For a requested plan, write only under `plans/` (or `animation-plans/` if `plans/` already exists for something else). User authorization to implement a selected change permits leaving this audit stage and completing that change; audit alone never authorizes implementation.
2. **No mutating operations during an audit-only task**, except requested plan files. No installs, builds with side effects, commits, or formatters. An authorized implementation stage follows the task's existing permissions, file boundaries, and verification requirements.
...
For a planning request that leaves the finding selection open, present the choices and wait for selection. Reuse an explicit selection already given, including a specified set such as all HIGH findings. An audit-only request is complete with the findings. For an implementation request, use the user's stated interface, behavior, and constraints as the scope; choose routine fixes within it and continue through implementation and verification. Ask only about unresolved choices that materially change the requested result or authorization, not whether the user wants the implementation already requested. If running non-interactively for plan creation, default to the top 3–5 by leverage.
...
| Invocation | Behavior |
| --- | --- |
| bare | Requested audit or planning scope: recon → audit → vet → reuse explicit selection or confirm → plans when requested |
| `quick` / `deep` | Adjust audit effort (see table); composes with a focus |
| a category focus (`performance`, `accessibility`, `easing`…) | Recon + audit that category only |
| `plan <description>` | Skip the audit; recon just enough to specify, then write a single plan for the described improvement |
| `execute <plan>` or an explicit request to implement selected findings | Complete the authorized implementation and verification in this task. An executor subagent in an isolated worktree may perform the implementation; review its result and deliver the completed work, not merely a handoff. |
