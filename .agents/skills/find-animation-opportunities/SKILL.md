...
description: Search a codebase or UI for useful animation opportunities and propose precise motion recipes. Use when the user requests suggestions or asks "what could be animated here?". Keep suggestion-only requests read-only; do not use this stage to replace an authorized implementation task.
---
...
1. **Keep suggestion-only requests read-only.** This stage reports opportunities. If the user authorizes building a suggestion, leave this stage and implement and verify the selected change in the same task, within the existing permissions. Do not require the user to repeat the request or take the recipe to another agent.
