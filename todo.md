...
> 状态：仅规划，暂不执行。
>
> 目标：为 Rust 股票多 Agent 投资研究系统增加 `-web` / `--web` 监控入口，构建“蒸汽机械铁路王国”风格的实时 3D 工作流看板，并将监控事件持久化到 FileStore JSONL 供实时展示和历史回放。
...
- [ ] 监控信息写入独立 FileStore JSONL 事件流，不与投资知识 Index/Detail 混写。
