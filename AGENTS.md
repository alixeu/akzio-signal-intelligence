...
| **Synthesizer Agent** | 综合证据与审查，生成 12 项预测提案 | `gpt-5.6-luna` (high) | 48k in / 5k out / 120s / 2 tools | `ArtifactKind::DecisionProposal` |
...
1. `read_document(artifact_id)`: 读取当前 Manifest 授权的完整规范化文档。
...
```text
DecisionProposal (LLM) 
       ↓ 
[DecisionGate (Rust)]    -> 校验 12 个 Forecast、风险上限、校准模型 -> 产出 Decision / DecisionContext
       ↓ 
...
在进行代码修改时，始终遵循窄步迭代与层层校验：

1. **窄域针对性测试**：优先运行所修改 crate 内部的具体测试模块。
2. **Workspace 级交付前验证**：
   ```bash
   cargo fmt --all
   ...
   ```
3. **不可随意破坏不变量**：重构或存储演进严禁隐式修改 schema_version、`ExecutionPlan` 序列化哈希、Paper Gate 风控逻辑或 Transaction 事务边界。
4. **交付物分级汇报**：明确标注文档与代码修改的四级验证状态：
   - `implemented` (代码实现)
   - `offline-verified` (本地编译/测试/Fixture 通过)
   - `real-Paper-verified` (Alpaca Paper 实盘沙箱验证)
   - `outcome/learning-verified` (T+5 周期闭环验证)

---

## 10. Observatory App 规范

- macOS 分发版本统一通过 `scripts/update_app_and_submit_debug.sh` 进行打包编译与签名。
- App Bundle 必须在 `Contents/MacOS/akzio-core` 内置编译后的 Rust core。
- Core 数据固定写入 `~/.akzio/store`，配置文件为 `~/.akzio/config.toml`。
- 界面提示 `Paper scheduler waiting: broker market is closed` 属于正常市场非交易状态，切勿当作 Core 服务异常。
...
- 40 项证据逐项留状态：执行安全失败仍 fail closed，研究和增强数据失败保留成功项与缺口。新鲜度看内容可用时间，日线以交易所收盘（美东 DST/提前收盘）加 20 分钟余量判断；不以下载时间或 UTC 日期替代 Session。
...
- 当前版本：Domain schema 10（兼容可选字段）；Store 15；正式 Contract 20；Prompt bundle 14；freshness candidate 21；Outcome metric `frozen_post_execution_exposure_v3`；evaluation context 1；benchmark definition 1。旧未知口径和 v2 数据不自动改标 v3。
...
修复证据与 34 项状态见 [任务一交接](docs/task1-repair-handoff.md)；隔离 Store、内部测试与入口见 [任务二 Debug](docs/task2-debug.md)，实际执行结果见 [离线 Debug 记录](docs/task2-offline-debug.md)。历史任务一检查不等于任务二结果，离线组合测试不等于真实 LLM/Paper 或实际跨交易日的学习验收。
