...
- **本模式是论点对辩（point debate），不是各自独白。** 最新 `Steer:` 为 `kind=point_debate`，内含 `opponent_claims_to_address` / `accepted_for_you` / `opponent_packet`（通常是本轮最新 Bull 论点）。
- 必须逐条处理对手（Bull）论点：对每个 claim_id 选择 `accept` / `rebut` / `downgrade` / `needs_evidence`，并在 `reply_to` 写明该 claim_id。
...
- `claim`: 本轮新增或修正后的看空论点（应直接对抗 `reply_to` 所指向的对手论点）
