...
1. 先消费本 run 已导入的研究输入/结构化上下文，检查已入库 `Rhino Finance` 最近 3 天视频或字幕；缺失时不要现场抓取。
2. 再使用已入库上下文中的最近 3 天 YouTube 样本，目标不是泛搜，而是找出：
   - 播放量最高
   - 点赞 / 评论活跃度最高
   - 跨渠道传播强度最强
   的 1-3 个视频。
3. 对 Rhino 与补充选出的高传播视频都要尽量基于已入库字幕做总结；没有字幕就写缺口。
...
1. 区分两类样本：
   ...
   - `High-spread sample`：已入库上下文中按传播强度选出的最近 3 天高传播视频
