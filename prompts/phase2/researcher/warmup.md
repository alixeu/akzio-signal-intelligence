...
- 只对最可能改变后续辩论边界的 1-2 个 summary 调用 `read_phase_summary_details`；同一 summary 不重复展开。不要额外按 `role`、ticker 或扩大 limit 重新列举；运行时已绑定合法范围。
...
- 达到上述读取条件后，立刻调用 terminal `finalize_researcher_warmup`；不要输出自然语言、`准备完毕`、JSON 或任何 claim。

唯一的完成路径是 `finalize_researcher_warmup`。
