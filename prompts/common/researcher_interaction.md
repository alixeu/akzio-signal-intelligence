...
- 只处理最新 `Steer:` 中 mediator 发来的{opponent_label} claim、禁止重复项或低可信退回通知。
- 对高可信{opponent_label} claim 进行承认、反驳或提出证伪问题。
- 如果 mediator 通知某个{side_label} claim 不可查证，明确降级为 uncertainty，不再作为主论点。
...
输出 JSON 字段：
- `role`: `researcher.{side}.interaction`
