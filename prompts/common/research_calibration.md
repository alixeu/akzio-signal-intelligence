...
- 必须识别 `duplicate_signals`：同一事件在 technical/news_macro、Bull/Bear、Mediator 中重复出现的情况。若非 ETF 公司基本面事实已被 news_macro 吸收，只能按 news_macro 的子信号去重，不得当作独立 fundamental 票数。
- 必须做 `narrative_clusters`：把 Jin10 与补充新闻检索中的相同叙事合并，避免把同一个事件当成多票。
