...
impl Daemon {
    pub(crate) async fn execute_shadow_evaluate(
        ...
        let Some(outcome_lease) = self.store.acquire_daemon_lease(
            &format!("{OUTCOME_WORKER_LEASE_NAME}:{}", task.run_id),
        ...
        let Some(stage) = collected
            .materialization
            .observations
            .iter()
            .find(|o| o.horizon == OutcomeHorizon::T5)
        else {
            return Ok(TaskCompletion::DeferredUntil(next_outcome_check_at(now)?));
        };
            ...
            .filter(|a| a.kind == ArtifactKind::NormalizedEvidence)
        {
            ...
            akzio_ingest::validate_outcome_price_window(
                &payload.value,
                parent_schedule.baseline_trading_day,
                stage.observed_trading_day,
            )
        ...
        let schedule_artifact = Artifact::new(
            ...
            self.store.stage_json(&candidate_schedule)?,
        ...
        for observation in &mut collected.materialization.observations {
            // Parent assessments evaluate the parent Decision and cannot be
            // reused as ground truth for a candidate Decision.
            observation.risk_recall = None;
        }
        ...
        let candidate_outcome = Artifact::new(
            ...
            self.store.stage_json(&candidate_outcome_payload)?,
