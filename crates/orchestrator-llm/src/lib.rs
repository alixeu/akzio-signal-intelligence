...
mod removed_json_output_path {
    ...
    fn stamp_runtime_artifact_envelope(
        settings: &AgentSettings,
        mut artifact: Value,
    ) -> Result<Value> {
        let object = artifact
    ...
    fn validate_json_artifact_contract(settings: &AgentSettings, artifact: &Value) -> Result<()> {
    ...
    fn validate_analyst_artifact_contract(
        settings: &AgentSettings,
        artifact: &Value,
    ) -> Result<()> {
        ...
        if actual_role == "analyst.news_macro" {
            ...
            if let Some(ids) = artifact.get("referenced_jin10_ids") {
                if !ids.is_array() && !ids.is_object() {
                    bail!("analyst.news_macro referenced_jin10_ids must be an array of jin10 ids or a map id->score");
                }
            }
        }
    ...
    fn validate_seed_packet_contract(settings: &AgentSettings, artifact: &Value) -> Result<()> {
    ...
    fn validate_topic_generation_contract(
        settings: &AgentSettings,
        artifact: &Value,
    ) -> Result<()> {
        require_exact_role(settings, artifact)?;
    ...
    fn validate_interaction_packet_contract(
        settings: &AgentSettings,
        artifact: &Value,
    ) -> Result<()> {
        require_exact_role(settings, artifact)?;
    ...
    fn validate_controller_packet_contract(
        settings: &AgentSettings,
        artifact: &Value,
    ) -> Result<()> {
        require_exact_role(settings, artifact)?;
    ...
    fn validate_trade_intent_contract(settings: &AgentSettings, artifact: &Value) -> Result<()> {
    ...
    fn validate_risk_constraints_contract(
        settings: &AgentSettings,
        artifact: &Value,
    ) -> Result<()> {
        validate_optional_role(settings, artifact)?;
    ...
    fn validate_optional_role(settings: &AgentSettings, artifact: &Value) -> Result<()> {
    ...
    fn require_exact_role<'a>(settings: &AgentSettings, artifact: &'a Value) -> Result<&'a str> {
        let role = artifact
            .get("role")
            .and_then(Value::as_str)
...
mod tests {
    ...
    #[cfg(any())]
    mod removed_json_output_contract_tests {
        use super::*;
        ...
        fn parse_final_output_enforces_analyst_direction_and_confidence() {
            ...
            assert!(
                err.to_string().contains("direction")
                    || err.to_string().contains("invalid analyst"),
