impl Store {
    ...
    fn verify_lesson_evidence(
        ...
        let mut statement = connection.prepare(
            "SELECT evidence_id, lesson_id, lesson_artifact_id, decision_context_artifact_id, outcome_artifact_id, attribution, metrics_json, recorded_at FROM rebuild_lesson_evidence ORDER BY evidence_id",
        )?;
        let rows = statement
            .query_map([], |row| {
                Ok((
        ...
        {
            let record = lesson_evidence_from_columns(
                lesson_id.clone(),
                lesson_artifact_id.clone(),
                decision_context_id.clone(),
                outcome_id.clone(),
                attribution.clone(),
                metrics_json,
                recorded_at.clone(),
            ...
            .map_err(|error| {
                StoreError::Integrity(format!(
                    "lesson evidence {evidence_id} has invalid persisted metrics: {error}"
                ))
            })?;
            let lesson_id = LessonId(lesson_id);
            if !head_ids.contains(&lesson_id) {
