...
fn read_run_kind_artifacts(
    ...
    let mut statement = connection.prepare("SELECT artifact_id FROM rebuild_artifacts WHERE json_extract(origin_json, '$.run_id') = ?1 AND kind = ?2 ORDER BY created_at, artifact_id")?;
    let ids = statement
        .query_map(params![run_id.0, enum_name(kind)], |row| {
            row.get::<_, String>(0)
        })?
        .collect::<Result<Vec<_>, _>>()?;
    ids.into_iter()
...
fn valid_retrospective_repair(
    ...
    old.horizon == OutcomeHorizon::T5
        ...
        && new.status == RetrospectiveStatus::Complete
        && old.outcome == new.outcome
        && old.outcome_id == new.outcome_id
        && previous.origin.as_ref().and_then(|o| o.run_id.as_ref())
            == next.origin.as_ref().and_then(|o| o.run_id.as_ref())
        && new.source_refs.contains(&ArtifactRef {
            artifact_id: previous.artifact_id.clone(),
            kind: ArtifactKind::Retrospective,
        })
        && next.source_refs == new.source_refs
}
