...
pub struct PolicyEvaluationCommit {
    /// False only while a fenced canary worker commits its remaining subjects.
    ...
    /// Observational Lesson ledger rows derived from this exact sealed Outcome.
    /// Store commits them atomically with evaluation and task success.
    pub lesson_evidence: Vec<LessonEvidence>,
    pub subject: PolicySubject,
...
/// Scheduling classes share the same durable task/attempt state machine.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Read-only projection of the two timelines; never a second stored state.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RunLifecycleHealth {
    pub execution_status: WorkflowStatus,
    pub outcome_scheduled: bool,
    pub numeric_outcome_sealed: bool,
    pub retrospective_status: std::collections::BTreeMap<String, String>,
    pub outcome_worker_failed: bool,
    pub narrative_repair_pending: bool,
    pub learning_recorded: bool,
    pub learning_eligible: Option<bool>,
    pub decision_production_usage: Option<RunModelUsage>,
    pub outcome_usage: RunModelUsage,
    pub lifecycle_usage: RunModelUsage,
}
