...
impl Store {
    pub fn verify_integrity(&self) -> StoreResult<()> {
        ...
        blob::verify_blob_storage(&connection)?;
        ...
        for (artifact_id, parent_id) in cross_run_references {
            let child = read_artifact(
                &connection,
            ...
            if !allowed {
                return Err(StoreError::Integrity(format!(
                    ...
                    child.kind, child.producer, parent_id, parent.kind, parent.producer
                )));
            }
        }
        ...
        let mut statement = connection.prepare(
            "SELECT commitment_artifact_id, asset, cancel_artifact_id, created_at \
             FROM rebuild_execution_cancels ORDER BY commitment_artifact_id, asset",
        )?;
        let cancels = statement
            .query_map([], |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, String>(1)?,
        ...
        for (commitment_artifact_id, asset, cancel_artifact_id, created_at) in cancels {
            ...
            let asset = Asset::try_from(asset.as_str())?;
            let commitment_artifact = read_artifact(&connection, &commitment_artifact_id)?;
            let cancel_artifact = read_artifact(&connection, &cancel_artifact_id)?;
            if commitment_artifact.kind != ArtifactKind::ExecutionCommitment
                || cancel_artifact.kind != ArtifactKind::ExecutionCancel
            {
                return Err(StoreError::Integrity(
            ...
            if cancel.commitment.artifact_id != commitment_artifact_id
                ...
                || !cancel_artifact
                    .source_refs
                    .iter()
                    .any(|source| source == &cancel.commitment)
                || !cancel_artifact
                    .source_refs
                    .iter()
                    .any(|source| source == &cancel.prior_receipt)
            {
                return Err(StoreError::Integrity(
                    "execution cancel provenance is invalid".to_owned(),
                ));
            }
            let prior_artifact = read_artifact(&connection, &cancel.prior_receipt.artifact_id)?;
            if prior_artifact.kind != ArtifactKind::OrderReceipt
                || !prior_artifact
                    .source_refs
                    .iter()
            ...
            {
                return Err(StoreError::Integrity(
                    "execution cancel prior receipt is invalid".to_owned(),
                ));
            }
            let prior: OrderReceipt =
                serde_json::from_slice(&blob::read_blob_with(&connection, &prior_artifact.blob)?)?;
            ...
            if prior.plan_hash != commitment.plan_hash
                ...
                || prior.client_order_id != cancel.client_order_id
                || prior.broker_order_id != cancel.broker_order_id
                || !commitment.client_order_ids.contains_key(&cancel.asset)
            {
                return Err(StoreError::Integrity(
                    "execution cancel receipt lineage is invalid".to_owned(),
                ));
            }
            let durable = connection
                .query_row(
            ...
            if durable.is_none() {
                return Err(StoreError::Integrity(
                    "execution cancel commitment is not durable".to_owned(),
                ));
            }
            parse_time(&created_at)?;
        }
