    ...
    pub fn release_evidence_bundle(
        ...
        let runtime = approval.as_ref().map(|(manifest, _)| {
            ...
            ReleaseRuntimeEvidence {
                repository_commit,
                dirty_worktree,
                cargo_lock_hash: manifest.cargo_lock_hash.clone(),
                ...
                decision_policy_hash: manifest.decision_policy_hash.clone(),
                execution_policy_hash: manifest.execution_policy_hash.clone(),
                evaluation_policy_hash: manifest.evaluation_policy_hash.clone(),
                behavior_bundle_hash: manifest.behavior_bundle_hash.clone(),
                experiment_profile: manifest.experiment_profile.clone(),
                rust_toolchain: manifest.rust_toolchain.clone(),
                model_knowledge_cutoff: manifest.model_knowledge_cutoff,
                market_data_feed: manifest.market_data_feed.clone(),
            }
        });
        ...
        let human_approval = approval
            ...
            .map(|(_, approval)| ReleaseHumanApprovalEvidence {
                ...
                is_post_outcome_research_approval: false,
            });
            ...
            .and_then(|artifact| {
                    ...
                    self.read_artifact_payload::<PostOutcomeResearchApproval>(&artifact)
                {
                    if payload.validate().is_ok() {
                        return Some(ReleasePostOutcomeApprovalEvidence {
                            approval_id: payload.approval_id,
                            decision: payload.decision,
                            operator_identity: payload.operator,
