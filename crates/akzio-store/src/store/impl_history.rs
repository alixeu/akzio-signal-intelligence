impl Store {
    fn verify_contract_catalogue_history(&self, connection: &Connection) -> StoreResult<()> {
        ...
        for (hash, artifact_id, contract_id, version, purpose, baseline) in installations {
            ...
            if let Some(baseline_hash) = &stored.baseline_contract_hash {
                ...
                let canonical_activation: bool = connection.query_row(
                    "SELECT EXISTS(SELECT 1 FROM rebuild_contract_activations WHERE contract_hash=?1 AND previous_contract_hash=?2 AND policy_transition_id IS NULL)",
                    params![contract_hash.as_str(),baseline_hash.as_str()], |row|row.get(0))?;
                if !(candidate_is_bounded(&baseline_contract.contract, &stored.contract)
                    || canonical_activation && workflow::canonical_context_repair_is_bounded(&baseline_contract.contract, &stored.contract))
                {
                    return Err(StoreError::Integrity(format!(
                ...
                (Some(previous_hash), None) => {
                    ...
                    if contract.baseline_contract_hash.as_ref() != Some(previous_hash)
                        ...
                        || !(candidate_is_bounded(&previous_contract.contract, &contract.contract)
                            || workflow::canonical_context_repair_is_bounded(&previous_contract.contract, &contract.contract))
                    {
                        return Err(StoreError::Integrity(format!(
    ...
    fn verify_policy_evaluation_history(&self, connection: &Connection) -> StoreResult<()> {
        ...
        let head_subjects = connection
            .prepare(
                "SELECT subject_id FROM rebuild_policy_consumption_heads ORDER BY subject_id",
            )?
            .query_map([], |row| row.get::<_, String>(0))?
            .collect::<Result<Vec<_>, _>>()?;
