...
impl ResolvedTechnicalArgs {
    fn from_args(args: TechnicalArgs) -> Result<Self> {
        let config = crate::cli_config::load_default_config().unwrap_or_else(|_| json!({}));
        ...
        Self {
            ...
            db_path: args
                .db_path
                .unwrap_or_else(|| crate::cli_config::shared_db_path_from_config(&config)),
            model: args
                .model
...
fn insert_feature_rows(
    ...
    let mut inserted = 0;
    for row in rows {
        let Some(table) = technical_table(&row.interval) else {
            continue;
        };
        ...
        for (name, value) in &row.features {
            ...
            conn.execute(
                ...
                params![row.symbol, row.date, name, value, model, payload, imported_at],
            )
...
fn technical_table(interval: &str) -> Option<&'static str> {
    match interval {
        "1d" => Some("technical_daily_indicators"),
        "3h" => Some("technical_3h_indicators"),
        "20min" => Some("technical_20min_indicators"),
        _ => None,
    }
}
...
fn has_fresh_rows(
    ...
    let Some(table) = technical_table(interval) else {
        return Ok(false);
    };
    let count: i64 = conn.query_row(
        &format!("SELECT COUNT(*) FROM {table} WHERE ticker = ? AND model = ? AND kline_time >= ?"),
        params![symbol, model, start.to_string()],
        |row| row.get(0),
    )?;
    Ok(count > 0)
}
