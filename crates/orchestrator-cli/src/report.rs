...
pub fn run(args: ReportArgs) -> Result<Value> {
    ...
    let report_dir = crate::cli_config::project_path_from_config(config_str(
        &config,
        "report.output_dir",
        "outputs/TQQQ",
    ));
...
fn build_payload(
    ...
    let run_dir = if let Ok(value) = std::env::var("CODEX_ORCH_RUN_DIR") {
        ...
        crate::cli_config::project_path_from_config(pattern.replace("{date}", today))
    };
...
fn send_report(payload: &Value) -> Result<Value> {
    ...
    let state_path = crate::cli_config::project_path_from_config(config_str(
        &config,
