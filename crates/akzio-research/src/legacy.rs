//! Legacy role-indexed research implementation awaiting owner-phase replacement.
//!
...
pub use crate::tools::{SourceRegistry, ToolCallContext, ToolExecution, ToolRuntime};
...
use akzio_context::legacy::{
    ContextBroker, ContextError, ContextManifest, ContextRequest, NewJsonDocument,
};
use akzio_domain::{
    content_hash_json, ContractId, DocumentId, DocumentKind, DocumentLifecycle, DocumentOrigin,
    DocumentRecord, FailureDisposition, LegacyAgentContract, LegacyDecisionDraft, Provenance,
    RetryPolicy, TaskBudget, TaskId, TaskKind, TaskSpec, TerminationPolicy, ToolGrant, ToolKind,
    TopologyId, WorkflowPlan, V2_SCHEMA_VERSION,
};
use akzio_model::{ModelClient, ModelError, ModelRequest};
use akzio_runtime::legacy::{PlanPatch, RuntimeError, WorkflowRuntime};
...
pub struct InstalledContract {
    pub role: AgentRole,
    pub contract: LegacyAgentContract,
...
impl OutputSpec {
    ...
    fn validate(self, output: &Value) -> Result<DocumentKind> {
        ...
        match self {
            ...
            Self::DecisionDraft => {
                let draft = serde_json::from_value::<LegacyDecisionDraft>(output.clone())
                    .map_err(|error| ResearchError::InvalidOutput(error.to_string()))?;
                draft
                    .validate()
                    .map_err(|error| ResearchError::InvalidOutput(error.to_string()))?;
...
fn install_contract(
    ...
    let contract = LegacyAgentContract {
...
async fn invoke_agent_with_tools(
    ...
    let definitions = if contract.budget.max_tool_calls > 0 {
...
pub async fn execute_research_task(
    ...
    let contract: LegacyAgentContract =
        serde_json::from_value(broker.read_json(&contract_record)?)?;
    contract
        .validate()
        .map_err(|error| ResearchError::InvalidOutput(error.to_string()))?;
