...
pub(super) fn validate_schema_value(
    ...
    if let Some(unique) = definition.get("uniqueItems") {
        if kind != "array" || !unique.is_boolean() {
            return Err(format!(
                "{path} schema.uniqueItems requires an array and boolean"
            ));
        }
        if unique.as_bool() == Some(true) {
            ...
            {
                return Err(format!("{path} violates schema.uniqueItems"));
            }
        }
    }
