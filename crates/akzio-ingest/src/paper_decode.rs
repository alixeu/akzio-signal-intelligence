...
use akzio_domain::{
    ...
    QuoteSnapshot, DOMAIN_SCHEMA_VERSION,
};
...
pub fn parse_daily_bars(
    ...
    for item in items {
        ...
        let date = item
            ...
            .ok_or_else(|| {
                PaperDecodeError::InvalidInput("daily bar timestamp missing or invalid".to_owned())
            })?;
        ...
        .map(|(symbol, quote)| {
            let asset = Asset::try_from(symbol.as_str()).map_err(|_| {
                PaperDecodeError::InvalidInput(format!(
                    "Paper quote asset outside executable universe: {symbol}"
                ))
            })?;
