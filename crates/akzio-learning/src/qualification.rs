...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReceiptBasedQualificationInput {
    pub key: ModelQualificationKey,
    pub frozen_context_manifest: ArtifactRef,
    pub champion_snapshot: ContentHash,
    pub challenger_snapshot: ContentHash,
...
pub fn run_model_qualification_from_receipts(
    input: &ReceiptBasedQualificationInput,
) -> Result<OfflineModelQualificationResult, QualificationRunError> {
    if input.receipts.is_empty() {
        return Err(QualificationRunError::InvalidInput("receipts"));
    }
    let mut scenarios = Vec::with_capacity(input.receipts.len());
    for receipt in &input.receipts {
        receipt
            .validate()
            .map_err(|_| QualificationRunError::InvalidEvidenceClosure)?;
        ...
        let champion_passed = !receipt.critical
            || receipt
                .measured_metrics
                .get("champion_passed")
                .copied()
                .unwrap_or(1)
                > 0;
        let challenger_passed = receipt.deterministic_verdict.is_pass();
        scenarios.push(QualificationScenarioObservation {
            scenario_id: receipt.scenario_id.clone(),
            stage: receipt.stage,
            critical: receipt.critical,
            champion_passed,
            challenger_passed,
            output_fingerprint: receipt.receipt_id.clone(),
        });
    }

    let offline_input = OfflineModelQualificationInput {
        key: input.key.clone(),
        frozen_context_manifest: input.frozen_context_manifest.clone(),
        champion_snapshot: input.champion_snapshot.clone(),
        challenger_snapshot: input.challenger_snapshot.clone(),
        rollback_snapshot: input.rollback_snapshot.clone(),
        observed_rollback_snapshot: input.observed_rollback_snapshot.clone(),
        required_scenarios: input.required_scenarios.clone(),
        scenarios,
        receipts: input.receipts.clone(),
        stage_evaluations: input.stage_evaluations.clone(),
        approved_by: input.approved_by.clone(),
        qualified_at: input.qualified_at,
        expires_at: input.expires_at,
    };
    run_offline_model_qualification(&offline_input)
}
